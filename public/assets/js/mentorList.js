...
$(document).ready(function(){
    ...
    $.get("/api/primaryhelp_timezone/" + selectPrimaryHelp + "/" + selectTimeZone)
      ...
      for (let i = 0; i < mentorData.length; i++) {
        ...
        const { mentor_name,  primary_help, institute_graduated, time_zone,description } = mentor;

        console.log(time_zone)
        console.log(institute_graduated)
         ...
         var mentorCard = $('<div class="card mb-4">');
          ...
          var mentorInfoStart = $('<div class="col-md-12">')
            var cardBody = $('<div class="card-body">')
              ...
              var timeZone = $('<h6 class="card-title" id="timeZone">').text('I graduated from: ' + time_zone)
              var descriptionDiv = $('<p class="card-text" id="description">').text(' ' + description)
          
            cardBody.append(mentorName, primaryHelp, schoolGraduated, timeZone,descriptionDiv);
